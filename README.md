...
Independent customer-support triage application powered by `augent-core`. Its
...
Admin telemetry use Augent Core's existing observability contracts.
...
the already classified ticket and approved knowledge result. Augent Core
...
The image runs as the non-root `augent` user. `.env`, local virtual
